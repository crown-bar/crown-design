...
>
	...
	>
		<DatePickerPrimitive.Input class="date-picker-input">
			{#snippet children({ segments })}
				{#each segments as { part, value: segmentValue }, i (i)}
					{#if part === 'literal'}
						<span class="date-field-literal">{segmentValue}</span>
					{:else}
						<DatePickerPrimitive.Segment {part} class="date-field-segment">
							{segmentValue}
		...
		<DatePickerPrimitive.Trigger class="date-picker-trigger" disabled={disabled || undefined} aria-label="Open calendar">
	...
	<DatePickerPrimitive.Portal>
		<DatePickerPrimitive.Content class="date-picker-content" sideOffset={6}>
